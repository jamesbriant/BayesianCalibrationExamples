...
submit_jobs() {
    ...
    local node_type="L" # Accepts any of E, F, J or L
    if [ "$mode" == "gpu" ]; then
        use_gpu=true
        job_mem=$GPU_MEM
    fi
    ...
    do
        JOB_SCRIPT_PATH="${TEMP_DIR}/sge_scripts/${node_type}_job_${d}.sge"
        RESULT_FILE_PATH="${TEMP_DIR}/results/${node_type}_result_${d}.csv"
        
        # Use a "here document" to write the SGE job script.
        cat > "$JOB_SCRIPT_PATH" << EOF
#!/bin/bash -l
#$ -N ${node_type}_${cleaned_name}_${d}_${mode}  # Job name
#$ -l h_rt=${JOB_TIME}
#$ -l mem=${job_mem}
# Add GPU request if needed
$( [ "$use_gpu" = true ] && echo "#$ -l gpu=1" )
#$ -ac allow=${node_type} # Accepts any of E, F, J or L
...
collect_results() {
    ...
    # Loop through each result file, prepend the gpu_used status, and append to the main CSV.
    for f in "${TEMP_DIR}/results/"*.csv
    do
        # check if file exists to avoid error with empty dir
        [ -e "$f" ] || continue
        result_data=$(cat "$f")
        echo "${result_data%,*},$use_gpu,${result_data##*,}" >> "$OUTPUT_FILE"
    done
