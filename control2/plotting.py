...
def gen_GP_test_data(
    ...
    GP_test = jnp.hstack((x_test, theta_arr))
    return GP_test
...
def gen_plot_test_data(
    ...
    theta_arr = jnp.tile(theta_vec, (x_test.shape[0], 1))
...
def plot_f_eta(
    ...
    x0_plot = x_test[:, 0]
    thetas_true = np.tile(true_params.get_theta(), (x0_plot.shape[0], 1))

    theta_label_components = [
        f"{theta:.2f}" for theta in thetas_test[0, :num_calib_params]
    ]
    theta_label = ", ".join(theta_label_components)

    ax.plot(
        x0_plot,
        eta(x_test, thetas_true),
        color="black",
        linestyle="--",
        label="True process",
    )
    ax.plot(x0_plot, eta(x_test, thetas_test), label=rf"$\eta(x, {theta_label})$")
    plot_GP(
        ax,
        test_GP[:, 0],
        GP_eta,
...
def plot_f_delta(
    ...
    ax.plot(
        x0_plot,
        zeta(x_test) - eta(x_test, thetas_test),
    ...
    ax.plot(
        x0_plot, delta(x_test), color="black", linestyle="--", label="True discrepancy"
    )
