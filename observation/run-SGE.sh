...
#    ./run_sge_timed_script.sh submit path/to/your_script.py linear cpu
#    ./run_sge_timed_script.sh submit path/to/your_script.py random gpu
#
# 2. Check job status until they are complete:
#    qstat
#
# 3. Once all jobs are done, collect the results:
#    ./run_sge_timed_script.sh collect path/to/your_script.py linear cpu
#    ./run_sge_timed_script.sh collect path/to/your_script.py random gpu
#
...
DIVISOR_LIST="200 100 66 50 40 33 25 20 15 12 10 8 6 4 2 1"
...
setup_names() {
    local obs_mode=$1 # linear or random
    ...
    OUTPUT_FILE="timings/timing_results_${cleaned_name}_${obs_mode}_${mode}.csv"
    # Construct a name for the directory that will hold temporary files.
    TEMP_DIR="sge_temp_${cleaned_name}_${obs_mode}_${mode}"
}
...
submit_jobs() {
    ...
    do
        JOB_SCRIPT_PATH="${TEMP_DIR}/sge_scripts/job_${obs_mode}_${d}.sge"
        RESULT_FILE_PATH="${TEMP_DIR}/results/result_${obs_mode}_${d}.csv"
        
        # Use a "here document" to write the SGE job script.
        cat > "$JOB_SCRIPT_PATH" << EOF
#!/bin/bash -l
#$ -N ${cleaned_name}_${d}_${obs_mode}_${mode}  # Job name
#$ -l h_rt=${JOB_TIME}
#$ -l mem=${job_mem}
...
#$ -wd /home/ucakjcb/Scratch/BayesianCalibrationExamples/observation
...
time_data=\$(/usr/bin/time -f "%e,%U,%S" python3 "$PYTHON_SCRIPT_ABSPATH" "$W" "$N" "$d" "$obs_mode" 2>&1 >/dev/null)
...
EOF
    ...
    echo "$0 collect $PYTHON_SCRIPT $obs_mode $mode"
    echo "--------------------------------------------------"
}
...
collect_results() {
    ...
    echo "Collecting results from '${TEMP_DIR}/' for '$obs_mode' with mode '$mode'..."
...
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 <submit|collect> <path_to_python_script.py> <linear|random> <cpu|gpu>"
    exit 1
fi
...
# Validate the observation mode argument.
if [[ "$OBS_MODE" != "linear" && "$OBS_MODE" != "random" ]]; then
    echo "Error: Invalid observation mode '$OBS_MODE'. Use 'linear' or 'random'."
    exit 1
fi
...
case "$COMMAND" in
    submit)
        submit_jobs "$OBS_MODE" "$MODE"
        ;;
    collect)
        collect_results "$OBS_MODE" "$MODE"
        ;;
    *)
