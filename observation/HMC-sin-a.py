...
from dataloader import save_chains_to_netcdf
...
def main():
    ...
    DATAFIELD = np.loadtxt(
        f"data/obs-{data_file_name}-{obs_mode}.csv", delimiter=",", dtype=np.float32
    )[::div, :]  # take every D-th observation data point
    DATACOMP = np.loadtxt(
    ...
    save_chains_to_netcdf(
        traces,
        traces_transformed,
        file_name=f"{file_name}-{obs_mode}",
